...
void LDC::detect(Mat srcimg, Mat& average_image, Mat& fuse_image)
{
	...
	for (int n = 0; n < num_outs; n++)
	{
		...
		accumulate(mask, average_image);  //������ͼ�����
		fuse_image = mask;
	}
	average_image = average_image / (float)num_outs; //���ƽ��ͼ��
	average_image.convertTo(average_image, CV_8UC1);
}

int main()
{
	LDC mynet("./LDC_640x360.onnx");
	string imgpath = "./IMG_2567.jpg";
