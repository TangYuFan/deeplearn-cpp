...
public:
	...
	Mat detect(Mat frame);  ///opencv4.6������������
	~HybridNets();  // ��������, �ͷ��ڴ�
private:
	int inpWidth;
	int inpHeight;
...
HybridNets::HybridNets(Net_config config)
{
	...
	this->net = readNet(config.modelpath);  ///opencv4.5����������ʱ�쳣��
...
Mat HybridNets::detect(Mat srcimg)
{
	...
	vector<Mat> outs;
	this->net.forward(outs, this->net.getUnconnectedOutLayersNames());   // ��ʼ����, opencv4.7�����������쳣��
	...
	const int num_proposal = outs[1].size[1];  ///������ǵ���ͼ, ��0άbatchsize����
	for (int n = 0; n < num_proposal; n++)
	{
		float conf = classification[n];
		/*int cls_id = -1;
		float max_conf = -10000;
		for (int k = 0; k < num_class; k++)   ////ֻ��car��һ���࣬û��Ҫ�����ֵ
		{
	...
	for (i = 0; i < outimg.rows; i++)
	{
		...
		{
			const int x = int(j / ratiow) + padw;  ///��ԭͼӳ��ص��������ͼ
...
int main()
{
	...
	Net_config cfg = { 0.3, 0.5, "./hybridnets_256x384.onnx", "./anchors_73656.bin" };
	HybridNets net(cfg);
