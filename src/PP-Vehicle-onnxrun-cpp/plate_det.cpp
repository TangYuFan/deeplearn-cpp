...
Mat PlateDetector::preprocess(Mat srcimg)
{
	Mat dstimg;
...
vector< vector<Point2f> > PlateDetector::detect(Mat& srcimg)
{
	int h = srcimg.rows;
	int w = srcimg.cols;
...
vector< vector<Point2f> > PlateDetector::order_points_clockwise(vector< vector<Point2f> > results)
{
...
void PlateDetector::draw_pred(Mat& srcimg, vector< vector<Point2f> > results)
{
	for (int i = 0; i < results.size(); i++)
	{
		for (int j = 0; j < 4; j++)
